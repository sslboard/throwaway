"""Deterministic browser-harness adapter for tempmailg.com."""

import json
import os

SERVICE = "tempmailg.com"
URL = os.environ.get("SERVICE_URL", "https://tempmailg.com/en")
...
(() => {
  ...
  if (select) {
    for (const option of Array.from(select.querySelectorAll('option'))) {
      ...
      if (text && DOMAIN_RE.test(text) && !text.includes('*')) {
        add('domain-option', text, '#name_domain option');
      }
    }
  }
