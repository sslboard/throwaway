"""Deterministic browser-harness adapter for emailtemp.org."""

import json
import os

SERVICE = "emailtemp.org"
URL = os.environ.get("SERVICE_URL", "https://emailtemp.org/en")
...
(() => {
  ...
  const input = document.querySelector('input#trsh_mail');
  if (input) add('generated-email-input', input.value || input.getAttribute('value') || '', '#trsh_mail');
...
try:
    ...
    if result["emails"]:
        ...
        if "captcha" in lowered or "verify you are human" in lowered or "cloudflare" in lowered:
            ...
            result["notes"].append("Loaded service but did not find a generated email address in input#trsh_mail.")
