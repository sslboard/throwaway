"""Deterministic browser-harness adapter for temp.tf."""
import json, os, re

SERVICE = "temp.tf"
URL = os.environ.get("SERVICE_URL", "https://temp.tf/")
...
try:
    ...
    for _ in range(8):
        ...
        data = js(r'''(() => {
          ...
          return {location: location.href, emails, text: text.slice(0, 1600)};
        })()''') or {}
    ...
    if result["emails"]:
        ...
        result["notes"].append("No generated email found on temp.tf.")
