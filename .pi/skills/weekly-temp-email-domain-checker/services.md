...
| `temp-inbox.me` | https://temp-inbox.me/ | adapter-ok | Adapter: `service-adapters/temp-inbox-me.py`; extracts inbox link email and parses `select#selected_domain` exposed domains. Verified 2026-08-28; exposed `gigacalulate.xyz` (added). |
...
| `yopmail.com` | https://yopmail.com/en/ | adapter-ok | Adapter: `service-adapters/yopmail-com.py`; forces `/email-generator`, extracts the generated address from `#egen`/`#geny`, and reads all options from `#seldom` inside iframe `#ifdoms`. Verified 2026-08-20 with 224 exposed domains. |
...
| `deepweb.net` | https://deepweb.net/temp-email | adapter-ok | Adapter: `service-adapters/deepweb-net.py`; combines rendered local-part input with the service's current domain. Verified 2026-08-28; generated `7uzmtq7olu@deep-mail.com` (added; service rotated from `deep-mail.org`). |
...
| `zenvex.dev` | https://zenvex.dev/ | adapter-ok | Adapter: `service-adapters/zenvex-dev.py`; extracts generated address from rendered body/control text and opens `.domain-trigger` to enumerate `[role="option"]` domains. Verified 2026-08-28; generated `ftqn8lb@souss.dev` (added). |
...
| `tempmail.lol` | https://tempmail.lol/en/ | adapter-ok | Adapter: `service-adapters/tempmail-lol.py`; extracts generated address from rendered body text. Verified 2026-08-28; generated `chandra78b19d@vrf.totalgamehub.net` (added). |
...
| `openinbox.io` | https://openinbox.io/ | adapter-ok | Adapter: `service-adapters/openinbox-io.py`; extracts generated landing-page address. Verified 2026-07-08; generated `inboxopen.space` (added). 2026-08-01 run found no generated email on the landing page. |
...
| `temp.tf` | https://temp.tf/ | adapter-ok | Adapter: `service-adapters/temp-tf.py`; reads the generated address from the rendered page. 2026-08-28 run rendered a `@outlook.com` address — legitimate Microsoft domain, NOT added to filter; treated as a service anomaly/false positive. |
...
| `internxt.com` | https://internxt.com/temporary-email | adapter-ok | Adapter: `service-adapters/internxt-com.py`; reads generated address from temporary-email widget button text. Verified 2026-08-28; generated `westcast-systems.com` (already covered). |
| `sandvpn.com` | https://sandvpn.com/mailbox | adapter-ok | Adapter: `service-adapters/sandvpn-com.py`; reads generated address from the address input. Verified 2026-08-28; generated `nobinbox.com` (added). |
| `usercheck.com` | https://www.usercheck.com/tools/disposable-email-checker | excluded | Signup-protection SaaS disposable-email-checker tool, not a generator. Added to `service-exclusions.json` 2026-08-28. |
| `zerobounce.net` | https://www.zerobounce.net/disposable-emails | excluded | Email-marketer guide on blocking disposable emails, not a generator. Added to `service-exclusions.json` 2026-08-28. |
