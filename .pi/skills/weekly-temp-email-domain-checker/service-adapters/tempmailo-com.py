"""Deterministic browser-harness adapter for tempmailo.com."""

import json
import os

SERVICE = "tempmailo.com"
URL = os.environ.get("SERVICE_URL", "https://tempmailo.com/")
...
(() => {
  ...
  // Tempmailo renders the generated address in readonly input#i-email.
  const input = document.querySelector('input#i-email');
  if (input) add('generated-email-input', input.value || input.getAttribute('value') || '', '#i-email');
