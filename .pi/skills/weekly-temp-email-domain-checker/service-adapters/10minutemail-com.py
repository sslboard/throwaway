"""Deterministic browser-harness adapter for 10minutemail.com."""

import json
import os

SERVICE = "10minutemail.com"
URL = os.environ.get("SERVICE_URL", "https://10minutemail.com/")
...
(() => {
  ...
  const input = document.querySelector('input#mail_address, input[aria-label="Your temporary email address"]');
  if (input) add('generated-email-input', input.value || input.getAttribute('value') || '', input.id ? `#${input.id}` : 'input[aria-label="Your temporary email address"]');
