"""Deterministic browser-harness adapter for 10minutemail.now."""
import json
import os

SERVICE = "10minutemail.now"
URL = os.environ.get("SERVICE_URL", "https://10minutemail.now/")

EXTRACT_QUERY = r"""
(() => {
  const EMAIL_RE = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi;
  const sel = document.querySelector('input#temp-email, input[id$="email"], input[type="text"]');
  const val = sel ? String(sel.value || '').trim() : '';
  const body = document.body ? document.body.innerText : '';
  const emails = [...new Set([...(val.match(EMAIL_RE) || []), ...((body.match(EMAIL_RE) || []))].map(s => s.toLowerCase()))];
  return {url: location.href, title: document.title, selVal: val, emails, body: body.slice(0, 800)};
})()
"""
...
try:
    ...
    for _ in range(12):
        ...
        emails = [e for e in (data.get("emails") or []) if not e.startswith(("support@", "no-reply@", "noreply@", "hello@"))]
        if emails:
            break
    ...
    if "captcha" in body or "verify you are human" in body:
        ...
        result["evidence"] = [{"kind": "input-value", "value": (data or {}).get("selVal", "")}] if (data or {}).get("selVal") else [{"kind": "body-email-text", "value": e} for e in emails[:5]]
