"""Deterministic browser-harness adapter for mails.org."""
import json
import os
SERVICE = "mails.org"
URL = os.environ.get("SERVICE_URL", "https://mails.org/")
EXTRACT_QUERY = r"""
(() => {
  ...
  const input = document.querySelector('input#generatedEmail');
  if (input) add('generated-email-input', input.value || input.getAttribute('value') || '', '#generatedEmail');
