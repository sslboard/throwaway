"""Deterministic browser-harness adapter for freecustom.email."""
import json, os, re

SERVICE = "freecustom.email"
URL = os.environ.get("SERVICE_URL", "https://www.freecustom.email/en")
...
try:
    ...
    for _ in range(8):
        ...
        generated = [e for e in data.get("emails", []) if not re.match(r"^(noreply@service\.com|inbox@ditapi\.info|abc123@fce\.email|service@example\.com)$", e)]
        if generated:
    ...
    if result["emails"]:
        ...
        result["notes"].append("No generated email found on freecustom.email.")
