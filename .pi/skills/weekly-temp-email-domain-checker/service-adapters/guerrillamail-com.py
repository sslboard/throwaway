"""Deterministic browser-harness adapter for guerrillamail.com."""

import json
import os

SERVICE = "guerrillamail.com"
URL = os.environ.get("SERVICE_URL", "https://www.guerrillamail.com/")
...
(() => {
  ...
  // Guerrilla Mail renders the generated address in page text and exposes available domains in select#gm-host-select.
  add('body-email-text', document.body ? document.body.innerText : '', 'document.body.innerText');

  const hostSelect = document.querySelector('select#gm-host-select');
  if (hostSelect) {
    add('selected-domain', hostSelect.value || '', '#gm-host-select');
    for (const option of Array.from(hostSelect.options || [])) {
      add('exposed-domain-option', option.value || option.textContent || '', '#gm-host-select option');
    }
  }


  const emails = uniq(sources.flatMap(s => s.value.match(EMAIL_RE) || []));
  const exposed_domains = hostSelect
    ? uniq(Array.from(hostSelect.options || []).map(option => option.value || option.textContent || ''))
    : [];
  return {
    ...
    emails,
    exposed_domains,
    evidence: sources.filter(s => EMAIL_RE.test(s.value)).slice(0, 20)
  };
})()
"""
