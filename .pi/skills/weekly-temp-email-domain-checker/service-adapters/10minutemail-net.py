"""Deterministic browser-harness adapter for 10minutemail.net."""
import json
import os
SERVICE = "10minutemail.net"
URL = os.environ.get("SERVICE_URL", "https://10minutemail.net/")
EXTRACT_QUERY = r"""
(() => {
  ...
  const input = document.querySelector('input#fe_text');
  if (input) add('generated-email-input', input.value || input.getAttribute('value') || '', '#fe_text');
